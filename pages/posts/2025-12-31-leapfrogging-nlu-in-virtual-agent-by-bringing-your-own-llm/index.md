...
2. Confusing — The NLU Workbench model has various capabilities, but they are laid out and organized in unintuitive ways. This makes it difficult to implement and difficult to distribute work among more than one person.
...
sequenceDiagram
    ...
    User->>VA: "I forgot my password"

    VA->>API: POST /classify
    Note right of VA: { utterance: "I forgot my password",<br/>topics: [...] }

    API->>API: Transform to prompt
    Note right of API: "Match this utterance<br/>to the most relevant topic..."

    API->>LLM: Chat completion request

    LLM->>API: Structured JSON response
    Note left of LLM: { topic_id: "pwd_reset",<br/>confidence: 0.95 }

    API->>VA: Topic match result

    VA->>User: Routes to Password Reset topic
```
...
```mermaid
flowchart TD
    A[LLM Returns Classification Result] --> B{Confidence Score}
    B -->|"≥ 0.7"| E[Route to Matched Topic]
    B -->|"< 0.7"| F["Return 'No Match Found'"]
    F --> G[Fallback to Human Agent]

    E --> H["0.95: 'forgot password' → Password Reset"]
    G --> I["0.45: 'something weird' → Agent Handoff"]
...
![Azure Infrastructure for LLM Topic Discovery](./azure-infrastructure.drawio.svg)

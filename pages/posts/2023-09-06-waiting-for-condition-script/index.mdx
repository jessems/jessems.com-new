...
date: "2023-09-06"
title: "Waiting for Condition Script"
description: "Waiting for Condition Script"
tags: "servicenow"
published: true
category: "technical"
---
...
How do you do this? ServiceNow's JavaScript engine, Rhino, doesn't support `setTimeout` and it runs on ES5 so it doesn't support `Promise`s or `async`/`await`.

There's an undocumented native API `gs.sleep()` which waiting function which blocks the current execution context, but it doesn't work in global scope.
